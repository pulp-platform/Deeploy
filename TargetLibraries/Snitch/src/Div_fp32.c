...
/*
 * Element-wise Division (FP32) with optional scalar broadcasting.
 *
 * is_scalar == 0:  output[i] = input1[i] / input2[i]
 * is_scalar != 0:  output[i] = input1[i] / input2[0]   (input2 read as scalar)
 *
 * The scalar branch precomputes inv_scalar = 1 / input2[0] once and turns the
 * loop into N multiplications instead of N divisions, which is faster on the
 * Snitch FPU.
 *
 * input1:    Numerator tensor (float32)
 * input2:    Denominator tensor (float32). Only input2[0] is read when
 *            is_scalar != 0.
...
void Div_fp32(float32_t *input1, float32_t *input2, float32_t *output,
  ...
  if (is_scalar) {
    float32_t inv_scalar = 1.0f / input2[0];
