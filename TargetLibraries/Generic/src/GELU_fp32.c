...
void GELU_fp32_fp32_sigmoid_grad_chunk(float32_t *grad_in, float32_t *data_in,
                                       ...
                                       int32_t end_idx) {
  // Exact GELU gradient: gelu'(x) = 0.5*(1+erf(x/sqrt(2))) +
  // x*exp(-0.5*x^2)/sqrt(2*pi) 1/sqrt(2)    = 0.70710678f 1/sqrt(2*pi) =
  // 0.39894228f
  //
  // The forward pass (GELU_fp32_fp32) uses the tanh-based exact GELU formula,
  // so the backward must match — the previous sigmoid-approximation gradient
  // was the GELU' for the sigmoid approximation, which produced inconsistent
  // gradients vs the (exact-tanh) forward path. Mirrors the upstream
  // Deeploy TrainingPlatform branch.
