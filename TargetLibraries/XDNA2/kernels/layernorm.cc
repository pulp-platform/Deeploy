...
template <typename T, int N> void layer_norm_bf16_bf16(const T *restrict input, T *restrict output, int32_t cols)
{
    event0();
...
void layer_norm_bf16_bf16_highacc(const bfloat16 *restrict input, bfloat16 *restrict output, int32_t cols)
{
    event0();
...
void layer_norm_bf16_bf16_highacc(bfloat16 *input, bfloat16 *output, int32_t cols)
{
    ::aie::set_rounding(aie::rounding_mode::conv_even);
    layer_norm_bf16_bf16_highacc(static_cast<const bfloat16 *>(input), output, cols);
}
