...
void PULP_AvgPoolGrad2d_fp32_fp32_HWC(
  ...
  for (uint32_t h_out = 0; h_out < H_out; ++h_out) {
    for (uint32_t w_out = 0; w_out < W_out; ++w_out) {
      ...
      if (h_end_pad > (int32_t)H_pad)
        h_end_pad = (int32_t)H_pad;
      if (w_end_pad > (int32_t)W_pad)
        w_end_pad = (int32_t)W_pad;
      ...
      if (actual_w_start < 0)
        actual_w_start = 0;
      if (actual_h_end > (int32_t)H_in)
        actual_h_end = (int32_t)H_in;
      if (actual_w_end > (int32_t)W_in)
        actual_w_end = (int32_t)W_in;
        ...
        for (uint32_t p = 0; p < P; ++p) {
          for (uint32_t q = 0; q < Q; ++q) {
            ...
            if ((uint32_t)in_h < H_in && (uint32_t)in_w < W_in) {
              uint32_t in_idx =
                  ((uint32_t)in_h * W_in + (uint32_t)in_w) * C + c;
