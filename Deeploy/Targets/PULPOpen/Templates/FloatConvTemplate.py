...
class PULP2DFloatConvGradWIm2ColTemplate(NodeTemplate):
    ...
    def computeTransientBuffersSize(
        ...
        # Using n_cores for potential parallelization
        im2col_dim = (operatorRepresentation["data_in_type"].typeWidth // 8) * operatorRepresentation[
            "n_cores"] * operatorRepresentation['ch_im_in'] * operatorRepresentation['dim_kernel_x'] * operatorRepresentation['dim_kernel_y']
...
for (uint32_t n=0; n<${batch}; ++n) {
    PULP_ConvGradX2d_fp${data_in_type.referencedType.typeWidth}_fp${weight_type.referencedType.typeWidth}_fp${data_out_type.referencedType.typeWidth}_NCHW_trainlib(
        ref_${data_out}_${data_in},
...
for (uint32_t n=0; n<${batch}; ++n) {
    PULP_ConvGradW2d_fp${grad_out_type.referencedType.typeWidth}_fp${data_in_type.referencedType.typeWidth}_fp${weight_type.referencedType.typeWidth}_NCHW_trainlib(
        ref_${weight}_${grad_out},
        ${dim_im_out_x}, ${dim_im_out_y}, ${ch_im_out},
        ref_${weight}_${data_in},
...
// 2D FP ConvGradW NCHW with Im2Col using pulp-trainlib (Name: ${nodeName}, Op: ${nodeOp})
${grad_out_type.typeName} ref_${weight}_${grad_out} = ${grad_out};
${data_in_type.typeName} ref_${weight}_${data_in} = ${data_in};
${weight_type.typeName} ref_${weight}_out = ${weight};

for (uint32_t n=0; n<${batch}; ++n) {
    PULP_ConvGradW2d_Im2Col_fp${grad_out_type.referencedType.typeWidth}_fp${data_in_type.referencedType.typeWidth}_fp${weight_type.referencedType.typeWidth}_NCHW_trainlib(
        ref_${weight}_${grad_out},
        ${dim_im_out_y}, ${dim_im_out_x}, ${ch_im_out},
        ref_${weight}_${data_in},
        ${dim_im_in_y}, ${dim_im_in_x}, ${ch_im_in},
