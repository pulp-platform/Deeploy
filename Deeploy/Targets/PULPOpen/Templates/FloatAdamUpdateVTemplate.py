...
// Adam UpdateV - First Moment Update, Parallel with 6x unrolling (Name: ${nodeName}, Op: ${nodeOp})
...
for (; i + 5 < ${nodeName}_chunk_stop; i += 6) {
    ...
    ${V_new}[i+0] = ${nodeName}_alpha * ${V}[i+0] + ${nodeName}_one_minus_alpha * ${nodeName}_G_reg_0;
    ${V_new}[i+1] = ${nodeName}_alpha * ${V}[i+1] + ${nodeName}_one_minus_alpha * ${nodeName}_G_reg_1;
    ${V_new}[i+2] = ${nodeName}_alpha * ${V}[i+2] + ${nodeName}_one_minus_alpha * ${nodeName}_G_reg_2;
    ${V_new}[i+3] = ${nodeName}_alpha * ${V}[i+3] + ${nodeName}_one_minus_alpha * ${nodeName}_G_reg_3;
    ${V_new}[i+4] = ${nodeName}_alpha * ${V}[i+4] + ${nodeName}_one_minus_alpha * ${nodeName}_G_reg_4;
    ${V_new}[i+5] = ${nodeName}_alpha * ${V}[i+5] + ${nodeName}_one_minus_alpha * ${nodeName}_G_reg_5;
}

for (; i < ${nodeName}_chunk_stop; i++) {
    float32_t ${nodeName}_G_reg = ${nodeName}_norm_coef * ${X}[i] + ${G}[i];
    ${V_new}[i] = ${nodeName}_alpha * ${V}[i] + ${nodeName}_one_minus_alpha * ${nodeName}_G_reg;
}
""")
