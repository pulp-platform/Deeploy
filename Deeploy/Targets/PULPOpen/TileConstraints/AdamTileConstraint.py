...
_TILED_TENSORS = ['R', 'T', 'X', 'G', 'V', 'H', 'X_new']
# Array tensors that share tile geometry with X (excludes the shape-(1,) scalar tensors R and T)
_ARRAY_INPUT_TENSORS = ['X', 'G', 'V', 'H']


class AdamTileConstraint(TileConstraint):
    ...
    def addPolicyConstraint(cls, tilerModel: TilerModel, parseDict: Dict,
                            ctxt: NetworkContext) -> TilerModel:
        # Fix every dimension except the first to its full size so the tiler
        # can only split along dim 0.
        xShape = ctxt.lookup(parseDict['X']).shape
    ...
    def serializeTilingSolution(
        ...
        # Shape-(1,) rectangle for the scalar inputs R and T (1 element each).
        scalar_cube = HyperRectangle((0,), (1,))

        for cube in outputCubes:
            tile_load = {name: cube for name in _ARRAY_INPUT_TENSORS}
            tile_load['R'] = scalar_cube
            tile_load['T'] = scalar_cube
            inputLoadSchedule.append(tile_load)

        for cube in outputCubes:
            outputLoadSchedule.append({'X_new': cube})
