...
class SILUTileConstraint(TileConstraint):
    ...
    def addGeometricalConstraint(tilerModel: TilerModel, parseDict: Dict, ctxt: NetworkContext) -> TilerModel:
        ...
        siluLUTBufferName = parseDict['silu_lut']

        for bufferName in [inputBuffer1Name, outputBufferName, siluLUTBufferName]:
    ...
    def serializeTilingSolution(
        ...
        addrNames = ['data_in', 'data_out', 'silu_lut']
        ...
        # LUT is loaded once and reused for all tiles (stays at same location in L1)
        lutName = operatorRepresentation['silu_lut']
        lutBuffer = ctxt.lookup(lutName)
        lutShape = lutBuffer.shape
        lutCube = HyperRectangle(tuple([0] * len(lutShape)), tuple(lutShape))

        # Each step includes both LUT (same offset) and data_in (different tile)
        # The LUT is loaded once and stays in place, data_in tiles are loaded per iteration
        for cube in outputCubes:
            inputLoadSchedule.append({"data_in": cube, "silu_lut": lutCube})
