...
"""
Pytest suite for Siracusa tiled gradient-operator kernel tests (single backward-pass ops, SBTiler).

Run all:
    pytest test_siracusa_grad_tiled.py -m siracusa_grad_tiled -v

Run one operator group:
    pytest test_siracusa_grad_tiled.py -k "SoftmaxGrad" -v

Skip simulation (codegen + build only):
    pytest test_siracusa_grad_tiled.py --skipsim -v
"""

import pytest

from test_siracusa_grad_tiled_config import DEFAULT_CORES, L2_SINGLEBUFFER_GRAD_KERNELS, PLATFORM_NAME, SIMULATOR
...
@pytest.mark.siracusa_grad_tiled
@pytest.mark.kernels
...
def test_siracusa_grad_tiled_kernels_l2_singlebuffer(test_params, deeploy_test_dir, toolchain, toolchain_dir,
