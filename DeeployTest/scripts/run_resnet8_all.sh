#!/usr/bin/env bash
# Run every ResNet8 variant on Siracusa/gvsoc with L3 spill and append
# rows to the master benchmark CSV. Resumable.
set -u
cd "$(dirname "$0")/.."
...
VARIANTS=( run_resnet8_nogcp run_resnet8_segsplit
           run_resnet8_bn run_resnet8_relu run_resnet8_bnrelu
           run_resnet8_conv run_resnet8_convbn run_resnet8_convadd
           run_resnet8_bn_kc
           run_resnet8_cost1 run_resnet8_cost2 run_resnet8_cost4
           run_resnet8_cost8 run_resnet8_cost12 run_resnet8_cost16
           run_resnet8_cost24 run_resnet8_cost30 )
echo "ResNet8 variants (${#VARIANTS[@]}): ${VARIANTS[*]}"
...
if name.startswith("resnet8_"):
    tag = name[len("resnet8_"):]
else:
    print(f"  [skip CSV emit]: {name}"); sys.exit(0)
test_dir = "ResNet8" if tag == "nogcp" else f"ResNet8_{tag.upper()}"
html = f"{dt}/{test_dir}/resnet8_train/deeployStates/memory_alloc.html"
...
row = [v, "RESNET8", tag, peak_l2(html), avg(fwd), avg(bwd), avg(accum),
       avg(opt), len(fwd), len(opt)]
with open(csv_out, "a", newline="") as f:
    csv.writer(f).writerow(row)
print(f"  -> ResNet8/{tag}: L2={row[3]} fwd={row[4]} bwd={row[5]} "
...
echo
echo "================================================================"
echo " ResNet8 sweep done   PASS=${#PASS[@]} SKIP=${#SKIP[@]} FAIL=${#FAIL[@]}"
echo " CSV: $CSV_OUT"
echo "================================================================"
