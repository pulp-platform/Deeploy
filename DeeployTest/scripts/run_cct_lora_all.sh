#!/usr/bin/env bash
# Run every CCT_LoRA variant on Siracusa/gvsoc with L3 spill.
set -u
cd "$(dirname "$0")/.."
...
VARIANTS=( run_cct_lora_nogcp run_cct_lora_segsplit
           run_cct_lora_ln run_cct_lora_softmax run_cct_lora_gelu
           run_cct_lora_ln_softmax run_cct_lora_ln_gelu run_cct_lora_korthikanti
           run_cct_lora_matmul run_cct_lora_ln_kc
           run_cct_lora_cost1 run_cct_lora_cost4 run_cct_lora_cost8
           run_cct_lora_cost16 run_cct_lora_cost24 run_cct_lora_cost32
           run_cct_lora_cost40 run_cct_lora_cost47 )
echo "CCT_LoRA variants (${#VARIANTS[@]}): ${VARIANTS[*]}"
...
if name.startswith("cct_lora_"):
    tag = name[len("cct_lora_"):]
else:
    print(f"  [skip CSV emit]: {name}"); sys.exit(0)
test_dir = "CCT_LoRA" if tag == "nogcp" else f"CCT_LoRA_{tag.upper()}"
...
row = [v, "CCT_LORA", tag, peak_l2(html), avg(fwd), avg(bwd), avg(accum),
       avg(opt), len(fwd), len(opt)]
with open(csv_out, "a", newline="") as f:
    csv.writer(f).writerow(row)
print(f"  -> CCT_LoRA/{tag}: L2={row[3]} fwd={row[4]} bwd={row[5]} "
...
echo
echo "================================================================"
echo " CCT_LoRA sweep done   PASS=${#PASS[@]} SKIP=${#SKIP[@]} FAIL=${#FAIL[@]}"
echo " CSV: $CSV_OUT"
echo "================================================================"
