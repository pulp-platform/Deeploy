#!/usr/bin/env bash
# Run every DSCNN variant on Siracusa/gvsoc and append rows to the master
# benchmark CSV. Resumable: skips variants already present in the CSV.
set -u
cd "$(dirname "$0")/.."
...
VARIANTS=( run_dscnn_nogcp run_dscnn_segsplit
           run_dscnn_bn run_dscnn_relu run_dscnn_bnrelu
           run_dscnn_conv run_dscnn_convbn
           run_dscnn_bn_kc
           run_dscnn_cost1 run_dscnn_cost2 run_dscnn_cost3 run_dscnn_cost5
           run_dscnn_cost8 run_dscnn_cost12 run_dscnn_cost16 run_dscnn_cost24 )
echo "DSCNN variants (${#VARIANTS[@]}): ${VARIANTS[*]}"
...
# Parse <key>_<tag>
if name.startswith("dscnn_"):
    tag = name[len("dscnn_"):]
else:
    print(f"  [skip CSV emit] not dscnn: {name}"); sys.exit(0)
# Test dir layout
test_dir = "DSCNN" if tag == "nogcp" else f"DSCNN_{tag.upper()}"
html = f"{dt}/{test_dir}/dscnn_train/deeployStates/memory_alloc.html"
...
row = [v, "DSCNN", tag, peak_l2(html), avg(fwd), avg(bwd), avg(accum),
       avg(opt), len(fwd), len(opt)]
with open(csv_out, "a", newline="") as f:
    csv.writer(f).writerow(row)
print(f"  -> DSCNN/{tag}: L2={row[3]} fwd={row[4]} bwd={row[5]} "
...
echo
echo "================================================================"
echo " DSCNN sweep done   PASS=${#PASS[@]} SKIP=${#SKIP[@]} FAIL=${#FAIL[@]}"
echo " CSV: $CSV_OUT"
echo "================================================================"
