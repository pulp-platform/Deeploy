#!/usr/bin/env bash
# CCT_LoRA in L2-RESIDENT mode (--defaultMemLevel L2, --l2 4M).
# Tests whether GCP wins when the whole footprint fits in L2 (no L3 streaming).
# CSV tag uses "l2_<variant>" to keep separate from the L3-spill rows.
set -u
cd "$(dirname "$0")/.."
...
VARIANTS=( run_cct_lora_l2_nogcp run_cct_lora_l2_segsplit
           run_cct_lora_l2_ln run_cct_lora_l2_softmax run_cct_lora_l2_gelu
           run_cct_lora_l2_ln_softmax run_cct_lora_l2_ln_gelu run_cct_lora_l2_korthikanti
           run_cct_lora_l2_matmul run_cct_lora_l2_ln_kc
           run_cct_lora_l2_cost1 run_cct_lora_l2_cost4 run_cct_lora_l2_cost8
           run_cct_lora_l2_cost16 run_cct_lora_l2_cost24 run_cct_lora_l2_cost32
           run_cct_lora_l2_cost40 run_cct_lora_l2_cost47
           run_cct_lora_l2_size128 run_cct_lora_l2_size256
           run_cct_lora_l2_size512 run_cct_lora_l2_size1024
           run_cct_lora_l2_size2048 run_cct_lora_l2_size4096
           run_cct_lora_l2_size8192 )
echo "CCT_LoRA L2-mode variants (${#VARIANTS[@]}): ${VARIANTS[*]}"
...
# name: cct_lora_l2_<variant>
prefix = "cct_lora_l2_"
if not name.startswith(prefix):
    print(f"  [skip CSV emit]: {name}"); sys.exit(0)
var = name[len(prefix):]
# Test dir uses the SAME variant ONNX as L3-mode (only runner flags differ).
test_dir = "CCT_LoRA" if var == "nogcp" else f"CCT_LoRA_{var.upper()}"
html = f"{dt}/{test_dir}/cct_lora_train/deeployStates/memory_alloc.html"
...
# Tag includes "l2_" prefix to distinguish from L3-spill rows in CSV.
row = [v, "CCT_LORA", f"l2_{var}", peak_l2(html), avg(fwd), avg(bwd), avg(accum),
       avg(opt), len(fwd), len(opt)]
with open(csv_out, "a", newline="") as f:
    csv.writer(f).writerow(row)
print(f"  -> CCT_LoRA/l2_{var}: L2={row[3]} fwd={row[4]} bwd={row[5]} "
...
echo
echo "================================================================"
echo " CCT_LoRA L2-mode sweep done   PASS=${#PASS[@]} SKIP=${#SKIP[@]} FAIL=${#FAIL[@]}"
echo " CSV: $CSV_OUT"
echo "================================================================"
