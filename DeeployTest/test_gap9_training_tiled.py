...
"""
Pytest suite for GAP9 tiled training tests (forward + backward + optimizer, SBTiler).

Run all:
    pytest test_gap9_training_tiled.py -m gap9_train_tiled -v

Run one test:
    pytest test_gap9_training_tiled.py -k "tinytransformer_train" -v

Skip simulation (codegen + build only):
    pytest test_gap9_training_tiled.py --skipsim -v
"""

import pytest

from test_gap9_train_tiled_config import DEFAULT_CORES, PLATFORM_NAME, SIMULATOR, TRAINING_TILED_TESTS
from testUtils.pytestRunner import create_training_test_config, run_and_assert_test


@pytest.mark.gap9_train_tiled
@pytest.mark.models
@pytest.mark.parametrize("test_name", list(TRAINING_TILED_TESTS.keys()), ids = list(TRAINING_TILED_TESTS.keys()))
def test_gap9_training_tiled(test_name, deeploy_test_dir, toolchain, toolchain_dir, cmake_args, skipgen,
                             skipsim) -> None:
